...
complex<double> ACompl;		//коэф ј
complex<double> BCompl;		//коэф ¬
complex<double> CCompl;		//коэф —
complex<double> Discr;		//дискриминант

string checkEquationCompl = "1234567890-+.i()";	//проверка на ошибки
string ErrorCheckCompl;		//ошибка

string sx1RCompl;		//действ. часть 1 корн€
string sx1ICompl;		//мним. часть 1 корн€
string sx2RCompl;		//действ. часть 2 корн€
string sx2ICompl;		//мним. часть 2 корн€
...
bool checkCompl(System::String^ s)		//проверка на ошибки
{
	...
	for (size_t i = 0; i < equation.length(); i++)
	{
		...
		for (size_t j = 0; j < checkEquationCompl.length(); j++)
			if (a == checkEquationCompl[j])
				yes = true;
		if (!yes)	//если нашелс€ запрещенный символ
		{
			ErrorCheckCompl = "«апрещенный символ";
			return false;
		}
		if (equation[i] == '.')		//если перед . нет числа
		{
			if (i == 0) 
			{
				ErrorCheckCompl = "ѕеред . нет числа";
				return false;
			}
		...
		if (i != equation.length() - 1)
		{
			...
			{
				ErrorCheckCompl = "„исло после i";
				return false;
			}
			if ((equation[i] == '-' || equation[i] == '+') && (equation[i + 1] == '-' || equation[i + 1] == '+'))	//++ -+ +- --
			{
				ErrorCheckCompl = "ѕроверьте знаки";
				return false;
			}
		}
	...
	if (iCounter > 1)
	{
		ErrorCheckCompl = "ѕроверьте кол-во мнимых чисел";
		return false;
	}
		
	if (signCounter > 2)
	{
		ErrorCheckCompl = "ћожет быть максимум 2 числа";
		return false;
	}
	if (signCounter == 2 && iCounter < 1)
	{
		ErrorCheckCompl = "Ќет мнимой части";
		return false;
	}
...
void TransformCompl(System::String^ s, char box)
{
	...
	string imag = "";	//мнима€ часть
	string real = "";	//действ часть

	string t1 = "";		//вспомогательна€ строка 1
	string t2 = "";		//вспомогательна€ строка 2
	...
	bool first = true;	//перва€ часть числа

	while (symb < eq.length())	//пока не дошла до конца строки
	{
		if (first)	//пока перва€ часть числа
		{
			if (symb == 0 && (eq[symb] == '+' || eq[symb] == '-'))	
			{
				if (eq[symb] == '-') //вычлен€ем знак первого числа 
					t1sign = -1;
				symb++;
			}
			else if (eq[symb] != '+' && eq[symb] != '-')
			{
				t1 += eq[symb];	//заполн€ем строку
				symb++;
			}
			else
		...
		else //если 2 часть числа
		{
			if (eq[symb] == '+' || eq[symb] == '-')
			{
				if (eq[symb] == '-')	//вычлен€ем знак 
					t2sign = -1;
				symb++;
			}
			else 
			{
				t2 += eq[symb];	//заполн€ем строку
				symb++;
			}
		}
	}
